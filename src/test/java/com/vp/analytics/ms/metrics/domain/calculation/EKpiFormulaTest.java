...
class EKpiFormulaTest {
    ...
    void shouldCalculateCorrectly() {
        BigDecimal averageTicket = EKpiFormula.AVERAGE_TICKET.calculate(DATA);
        assertEquals(new BigDecimal("5000.00"), averageTicket);

        BigDecimal netResult = EKpiFormula.NET_RESULT.calculate(DATA);
        assertEquals(new BigDecimal("15000"), netResult);

        BigDecimal cac = EKpiFormula.CAC.calculate(DATA);
        assertEquals(new BigDecimal("2500.00"), cac);

        BigDecimal ltv = EKpiFormula.LTV.calculate(DATA);
        assertEquals(new BigDecimal("60000.00"), ltv);
    }

    @Test
    void shouldFailCalculationNecessaryDataNotGiven() {
        BigDecimal averageTicket = EKpiFormula.AVERAGE_TICKET.calculate(MISSING_DATA);
        assertEquals(BigDecimal.ZERO, averageTicket);

        BigDecimal netResult = EKpiFormula.NET_RESULT.calculate(MISSING_DATA);
        assertEquals(new BigDecimal("-75000"), netResult);

        BigDecimal cac = EKpiFormula.CAC.calculate(MISSING_DATA);
        assertEquals(BigDecimal.ZERO, cac);

        BigDecimal ltv = EKpiFormula.LTV.calculate(MISSING_DATA);
